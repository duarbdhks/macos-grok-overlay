...
def main():
    ...
    print()
    # Rename the process so Activity Monitor / `ps` show "Grok Overlay" instead of "Python".
    try:
        import setproctitle
        setproctitle.setproctitle(f"{APP_TITLE} Overlay")
    except ImportError:
        pass
    # Override the bundle name so the menu bar / Dock display "Grok" instead of "Python".
    try:
        ...
        if _bundle is not None:
            ...
            if _info is not None:
                _info["CFBundleName"] = APP_TITLE
                _info["CFBundleDisplayName"] = APP_TITLE
                _info["CFBundleExecutable"] = APP_TITLE
    except Exception:
        pass
